...
if ($action === 'login') {
    ...
    try {
        $u = db()->prepare('UPDATE users SET last_login_at = ? WHERE id = ?');
        $u->execute([date('Y-m-d H:i:s'),(int)$user['id']]);
...
if ($action === 'register') {
    ...
    $password = (string)($data['password'] ?? '');
    if ($name === '' || $email === '' || $username === '' || strlen($password) < 4) {
        json_response(['ok'=>false,'error'=>'Preencha nome, e-mail, usuário e uma senha com pelo menos 4 caracteres.'], 422);
    }
...
if ($action === 'library') {
    ...
    json_response(['ok'=>true,'tracks'=>$tracks,'sync'=>$sync,'account'=>native_account_payload($user),'csrf'=>csrf_token()]);
}
