...
require_once __DIR__ . '/official_data_policy.php';
...
function ep7_context_stats(): array
{
    ...
    $out['road_data_mode']=epc_official_data_only()?'OFFICIAL':'COMMUNITY';
    return $out;
}

function ep7_build_context(array $body,string $deviceToken=''): array
{
    ep7_ensure_schema();ep7_housekeeping(false);$lat=(float)($body['lat']??0);$lon=(float)($body['lon']??0);if(!ep7_valid_coord($lat,$lon))throw new InvalidArgumentException('Coordenada inválida.');$route=ep7_route_points($body['route_points']??[],$lat,$lon);$speed=(float)($body['speed_kmh']??80);$include=is_array($body['include']??null)?array_map('strtolower',$body['include']):['hazards','live','traffic','weather','fuel'];$want=static fn(string $x):bool=>in_array($x,$include,true);$officialOnly=epc_official_data_only();
    $trafficOptIn=!$officialOnly&&ep7_parse_bool($body['traffic_opt_in']??false,false);
    $trafficSample=['stored'=>false,'reason'=>$officialOnly?'official_only_mode':'not_requested'];if($trafficOptIn&&$deviceToken!=='')$trafficSample=ep7_store_traffic_sample($body,$deviceToken);
    $result=['version'=>ESTRADAPLAY_CONTEXT_VERSION,'generated_at'=>date(DATE_ATOM),'road_data_mode'=>$officialOnly?'OFFICIAL':'COMMUNITY','official_sources'=>epc_official_source_markers(),'route'=>['points'=>count($route),'distance_m'=>(int)round(ep7_route_distance($route))],'privacy'=>['traffic_opt_in'=>$trafficOptIn,'device_id_stored'=>false,'traffic_retention_h'=>2],'traffic_sample'=>$trafficSample];
    if($want('hazards')){
        $hazards=ep7_hazards($route,(float)max(800,min(3000,(int)($body['hazard_corridor_m']??1800))),240);
        if($officialOnly)$hazards=array_values(array_filter($hazards,static fn(array $h):bool=>epc_source_is_official((string)($h['source']??''))));
        $result['hazards']=$hazards;
    }
    if($want('live'))$result['live_events']=$officialOnly?[]:ep7_live_events($route,(float)max(1500,min(12000,(int)($body['live_corridor_m']??6000))),100);
    if($want('traffic'))$result['traffic']=$officialOnly?[]:ep7_traffic_context($route,(float)max(1500,min(8000,(int)($body['traffic_corridor_m']??3500))),50);
    if($want('weather'))$result['weather']=ep7_route_weather($route,$speed,5);
    if($want('fuel'))$result['fuel']=$officialOnly?[]:ep7_fuel_context($route,(string)($body['fuel']??''),(float)($body['consumption_km_l']??10),(float)($body['liters_to_buy']??30),12);
