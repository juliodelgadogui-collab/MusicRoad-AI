...
final class RoadProtectionStatusUiV303 implements Application.ActivityLifecycleCallbacks {
    ...
    private final BroadcastReceiver receiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            ...
            if (available) hide(a); else show(a, "GPS sem sinal confiável · tentando recuperar");
        }
    };
    ...
    private final Runnable staleCheck = new Runnable() {
        @Override public void run() {
            ...
            if (a instanceof RoadMapActivity && !a.isFinishing()) {
                ...
                if (lastStateAt > 0L && age >= UI_STALE_MS) show(a, "Sem atualização da proteção · tentando recuperar");
                main.postDelayed(this, 5_000L);
            }
        }
    };

    private void show(Activity a, String detail) {
        ...
        FrameLayout.LayoutParams lp=new FrameLayout.LayoutParams(Math.min(dp(a,440),Math.max(dp(a,280),sw-dp(a,36))),-2,Gravity.TOP|Gravity.CENTER_HORIZONTAL);
        lp.topMargin=dp(a,76);
        host.addView(card,lp);
        if(Build.VERSION.SDK_INT>=21)card.setElevation(dp(a,110));
    }

    private void hide(Activity a) {
        FrameLayout host=host(a);if(host==null)return;View v=host.findViewWithTag(TAG);if(v!=null)host.removeView(v);
    }
    ...
    @Override public void onActivityPaused(Activity activity) {if(resumed.get()==activity){hide(activity);resumed=new WeakReference<>(null);main.removeCallbacks(staleCheck);}}
