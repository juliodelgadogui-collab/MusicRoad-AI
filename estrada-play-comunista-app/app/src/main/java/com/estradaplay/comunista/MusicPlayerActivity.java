...
public final class MusicPlayerActivity extends ComponentActivity {
    ...
    private void renderTracks(List<Track> tracks){if(listBox==null)return;while(listBox.getChildCount()>1)listBox.removeViewAt(1);int n=tracks==null?0:tracks.size();if(count!=null)count.setText(n+" "+(n==1?"música offline":"músicas offline"));
        if(n==0){LinearLayout empty=col();empty.setPadding(0,dp(20),0,0);empty.addView(text("Nenhuma música reconhecida",19,TEXT,true));empty.addView(text("Se você já tinha músicas baixadas, toque em LIMPEZA para reindexar ou recuperar cópias de versões antigas.",12,MUTED,false));Button repair=primary("PROCURAR MÚSICAS NO CELULAR");LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,dp(54));rp.setMargins(0,dp(18),0,0);empty.addView(repair,rp);repair.setOnClickListener(v->openStorage());Button add=small("ADICIONAR / BAIXAR MÚSICAS");LinearLayout.LayoutParams ap=new LinearLayout.LayoutParams(-1,dp(50));ap.setMargins(0,dp(10),0,0);empty.addView(add,ap);add.setOnClickListener(v->openDownloads());listBox.addView(empty);return;}
