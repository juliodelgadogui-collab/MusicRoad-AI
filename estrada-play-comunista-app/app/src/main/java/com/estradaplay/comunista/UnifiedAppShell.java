...
/**
 * Estrada Play Premium shared shell.
 * Secondary screens get one quiet top bar and the same importable theme. No fixed bottom navigation.
 */
final class UnifiedAppShell {
    ...
    static View wrap(Activity a, String section, View content) {
        ...
        LinearLayout.LayoutParams mp = new LinearLayout.LayoutParams(dp(a, 42), dp(a, 42)); mp.setMargins(dp(a, 8),0,0,0); bar.addView(mark, mp);
        ...
        LinearLayout.LayoutParams wp = new LinearLayout.LayoutParams(0, -1, 1f); wp.setMargins(dp(a,10),0,0,0); bar.addView(words, wp);
    ...
    private static void navigate(Activity a, int index) {
        ...
        else if (index == 1) cls = MusicPlayerActivity.class;
        else if (index == 2) cls = MusicStorageActivity.class;
    ...
    private static void polish(Activity a, View v, EstradaTheme theme) {
        if (v instanceof Button) {
            ...
            boolean primary = raw.contains("SALVAR") || raw.contains("REGISTRAR") || raw.contains("ATIVAR")
                    || raw.contains("INICIAR") || raw.contains("CONECTAR") || raw.contains("FALAR")
                    || raw.contains("CONFIRMAR");
            b.setBackground(PremiumUi.panel(a, primary ? theme.primary : theme.surfaceAlt,
                    primary ? theme.primary : theme.border, theme.radiusDp));
