...
public final class RoadReportActivity extends ComponentActivity {
    ...
    @Override protected void onStart(){super.onStart();rx=new BroadcastReceiver(){@Override public void onReceive(Context c,Intent i){lat=i.getDoubleExtra("lat",Double.NaN);lon=i.getDoubleExtra("lon",Double.NaN);speedKmh=i.getDoubleExtra("speed_kmh",0);if(gpsState!=null){boolean ok=Double.isFinite(lat)&&Double.isFinite(lon);gpsState.setText(ok?(speedKmh>=3?"GPS pronto · "+Math.round(speedKmh)+" km/h":"GPS pronto"):"Aguardando GPS…");gpsState.setTextColor(ok?GREEN:GOLD);}}};IntentFilter f=new IntentFilter(RoadSafetyService.ACTION_STATE);if(Build.VERSION.SDK_INT>=33)InternalBroadcasts.register(this, rx, f);else InternalBroadcasts.register(this, rx, f);reg=true;}
