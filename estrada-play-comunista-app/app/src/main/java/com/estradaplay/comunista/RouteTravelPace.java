...
final class RouteTravelPace {
    ...
    static synchronized void observe(double speedKmh, long nowMs) {
        ...
        if (lastAt > 0 && nowMs - lastAt > 90_000L) {
            // A long break starts a new pace window instead of carrying old congestion forever.
    ...
    static synchronized double estimateRemaining(double remainingM, double baselineSeconds) {
        if (!Double.isFinite(remainingM) || remainingM <= 0 || !Double.isFinite(baselineSeconds) || baselineSeconds <= 0) {
            return Math.max(0, baselineSeconds);
        }
        ...
        // Trust the live pace gradually; provider ETA remains the anchor.
