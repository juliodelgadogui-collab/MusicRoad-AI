...
/**
 * Local context engine for the Estrada Play Patriota voice.
 ...
 */
final class PatriotaCopilot {
    private static final String PREFS = "epp_copilot_v1";
    private static final String KEY_LAST_COMPATRIOT = "last_compatriot_ms";
    private static final long COMPATRIOT_GAP_MS = 150_000L;

    private final SharedPreferences prefs;
    private final AtomicInteger sequence = new AtomicInteger();

    PatriotaCopilot(Context context) {
    ...
    String roadLimit(int limitKmh) {
        ...
        return maybeCompatriot(variants[n % variants.length], false, n);
    }
    ...
    String overspeed(int limitKmh) {
        ...
        return maybeCompatriot(variants[n % variants.length], true, n);
    }
    ...
    String hazard(String type, double forwardM, int limitKmh) {
        ...
        return maybeCompatriot(text, true, n);
    }
    ...
    String routeStarted(String destination) {
        ...
        return maybeCompatriot(base, false, n);
    }
    ...
    private String maybeCompatriot(String text, boolean urgent, int n) {
        long now = System.currentTimeMillis();
        long last = prefs.getLong(KEY_LAST_COMPATRIOT, 0L);
        boolean due = now - last >= COMPATRIOT_GAP_MS;
        // Never force the word into every warning. Even when due, only some events use it.
        boolean use = due && ((urgent && n % 5 == 0) || (!urgent && n % 3 == 0));
        if (!use) return text;
        prefs.edit().putLong(KEY_LAST_COMPATRIOT, now).apply();
        return "Compatriota, " + lowerFirst(text);
    }
