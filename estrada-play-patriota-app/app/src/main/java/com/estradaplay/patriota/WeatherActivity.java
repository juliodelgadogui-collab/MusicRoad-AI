...
public final class WeatherActivity extends ComponentActivity {
    ...
    private void build(){
        ...
        LinearLayout head=row();head.setGravity(Gravity.CENTER_VERTICAL);Button back=button("‹ CENTRAL",false);head.addView(back,new LinearLayout.LayoutParams(dp(100),dp(46)));back.setOnClickListener(v->finish());LinearLayout title=col();title.addView(over("CLIMA DA ROTA",GOLD));title.addView(text("Chuva à frente",27,TEXT,true));title.addView(text("Previsão local + pontos da rota calculada.",11,MUTED,false));head.addView(title,new LinearLayout.LayoutParams(0,-2,1));page.addView(head);

        RoadWeatherMonitor.Snapshot s=RoadWeatherMonitor.snapshot(this);
        LinearLayout hero=card();hero.addView(over(s.headline(),s.currentWet?RED:(s.routeRisk()||s.nextRainMinutes>=0?GOLD:GREEN)));hero.addView(text(RoadWeatherMonitor.compactStatus(this),21,TEXT,true));
        ...
        page.addView(section("AGORA E PRÓXIMAS HORAS","A posição vem do GPS. A previsão vem do Open-Meteo."));
        LinearLayout local=card();
        local.addView(metric("PISTA",s.available?(s.currentWet?"MOLHADA / CHUVA":"SEM CHUVA AGORA"):"SEM DADOS",s.currentWet?RED:GREEN));
        local.addView(metric("CHANCE AGORA",s.available?(s.currentChance+"% "):"—",GOLD));
        local.addView(metric("PRÓXIMA CHUVA",s.nextRainMinutes>=0?("~"+s.nextRainMinutes+" min · "+s.nextRainChance+"%"):"não prevista em 3 h",s.nextRainMinutes>=0?GOLD:GREEN));
        add(page,local,0,8,0,16,-1,-2);

        page.addView(section("AO LONGO DA ROTA","Depois de calcular uma viagem, o Estrada Play verifica pontos progressivos do percurso."));
        ...
        if(s.routeRisk()){
            ...
            route.addView(over("ROTA MONITORADA",GREEN));route.addView(text("Sem chuva relevante detectada nos pontos analisados.",15,TEXT,true));route.addView(text("Destino: "+s.routeLabel,10,MUTED,false));
        }else{
            route.addView(over("SEM ROTA ATIVA",MUTED));route.addView(text("Calcule uma viagem para eu verificar chuva no caminho.",14,TEXT,true));
        }
        ...
        Button toggle=button(DriveSettings.autoRain(this)?"CHUVA AUTOMÁTICA · ATIVA":"CHUVA AUTOMÁTICA · DESLIGADA",false);add(page,toggle,0,0,0,16,-1,dp(50));toggle.setOnClickListener(v->{boolean n=!DriveSettings.autoRain(this);DriveSettings.toggle(this,"rain_auto",n);toast(n?"Clima automático ativado":"Clima automático desativado");build();});

        LinearLayout explain=card();explain.addView(over("COMO O APP DECIDE",MUTED));explain.addView(text("• chuva atual: precipitação e código meteorológico da posição GPS\n• próximas horas: probabilidade + precipitação prevista\n• rota: até quatro pontos do percurso, comparados com o horário estimado de chegada\n• sem resposta do provedor, o app mostra sem dados e não inventa chuva",11,TEXT,false));add(page,explain,0,0,0,0,-1,-2);
        if(!s.error.isEmpty()){TextView e=text("Última falha de atualização: "+s.error,9,MUTED,false);add(page,e,2,8,0,0,-1,-2);}
    }

    private void refresh(Button b){if(!DriveSettings.autoRain(this)){toast("Ative Chuva Automática primeiro.");return;}Location l=last();if(l==null){toast("Ainda não tenho uma posição GPS válida.");return;}b.setEnabled(false);b.setText("ATUALIZANDO…");io.execute(()->{RoadWeatherMonitor.refresh(getApplicationContext(),l.getLatitude(),l.getLongitude());runOnUiThread(()->{b.setEnabled(true);toast("Clima atualizado");build();});});}
