...
rm -rf /tmp/epc205-voice && mkdir -p /tmp/epc205-voice
tar -xjf "$MODEL" -C /tmp/epc205-voice
python3 "$APP/tools/generate_voice_bank.py" --model-root /tmp/epc205-voice --output "$APP/app/src/main/res/raw" --metadata-output "$APP/app/src/main/assets/estradaplay_voice"
...
test -f "$A"
"$ANDROID_HOME/build-tools/35.0.0/aapt" dump badging "$A" | tee /tmp/epc205-badging.txt
grep -q "package: name='com.estradaplay.comunista.universal' versionCode='205' versionName='2.0.5'" /tmp/epc205-badging.txt
"$ANDROID_HOME/build-tools/35.0.0/apksigner" verify --print-certs "$A" >/tmp/epc205-cert.txt
cp "$A" "$OUT"
sha256sum "$OUT" | tee /tmp/epc205-sha.txt
