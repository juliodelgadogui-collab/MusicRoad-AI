...
def replace_required(text: str, old: str, new: str, label: str):
    if old not in text:
        raise SystemExit(f"missing {label}: {old!r}")
    return text.replace(old, new)

# Version only. No feature/server changes in this patch.
g = GRADLE.read_text(encoding="utf-8")
g = re.sub(r"versionCode\s+171\b", "versionCode 172", g)
g = re.sub(r"versionName\s+'1\.7\.1'", "versionName '1.7.2'", g)
write(GRADLE, g)

shell = r'''package com.estradaplay.comunista;
...
'''
write(J / "UnifiedAppShell.java", shell)

# Map every secondary screen into one of the five visible sectors.
targets = {
    "MusicPlayerActivity.java": "music",
    "RoadRadioActivity.java": "radio",
    "TripPlannerActivity.java": "trip",
    "DestinationActivity.java": "trip",
    "TripHistoryActivity.java": "trip",
    "TripReplayActivity.java": "trip",
    "VehicleCostActivity.java": "trip",
    "RoadFavoritesActivity.java": "trip",
    "DriveToolsActivity.java": "central",
    "OfflineCenterActivity.java": "central",
    "CollectiveRoadActivity.java": "central",
    "NearbyServicesActivity.java": "central",
    "RoadReportActivity.java": "central",
    "VoiceCommandActivity.java": "central",
    "VoiceDiagnosticsActivity.java": "central",
    "ServerSettingsActivity.java": "central",
    "ThoughtsActivity.java": "central",
    "VideoLibraryActivity.java": "central",
}
...
for name, section in targets.items():
    ...
    if "UnifiedAppShell.wrap" in s:
        continue
    # Activities in this codebase construct a root View in a local variable and then call setContentView(root).
    # Wrap every simple-variable setContentView call so rebuild() methods also keep the shell.
    s2, n = re.subn(r"setContentView\(([A-Za-z_][A-Za-z0-9_]*)\);", rf'setContentView(UnifiedAppShell.wrap(this,"{section}",\1));', s)
    if n == 0:
        raise SystemExit(f"no simple setContentView found in {name}")
    write(p, s2)

# Make the cockpit rail use the exact same five sectors as every secondary screen.
road = J / "RoadMapActivity.java"
r = road.read_text(encoding="utf-8")
rail_method = r'''    private LinearLayout buildRail(int height, boolean compact) {
...
'''
r2, n = re.subn(r"    private LinearLayout buildRail\(int height, boolean compact\) \{.*?\n    \}\n(?=\n    private )", rail_method, r, count=1, flags=re.S)
if n != 1:
    raise SystemExit("could not replace RoadMapActivity.buildRail")
r2 = r2.replace('Button music = action("SOM", true);', 'Button music = action("MÚSICA", true);')
r2 = r2.replace('centralButton.setOnClickListener(v -> openMain("home"));', 'centralButton.setOnClickListener(v -> startActivity(new Intent(this, DriveToolsActivity.class)));')
write(road, r2)

print("Universal 1.7.2 unified shell applied")
