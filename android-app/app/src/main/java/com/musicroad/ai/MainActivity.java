...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver playbackReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            ...
            String json = intent.getStringExtra(PlaybackService.EXTRA_STATE_JSON);
            if (json == null) return;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            if (url.contains("SEU-DOMINIO")) {
                Toast.makeText(this,"Configure MUSICROAD_URL em gradle.properties. A biblioteca nativa continua disponível offline.",Toast.LENGTH_LONG).show();
                webView.loadUrl("file:///android_asset/offline.html?unconfigured=1");
    ...
    private void configureWebView() {
        ...
        s.setUserAgentString(s.getUserAgentString()+" MusicRoadAndroid/2.0");
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            @Override public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
                ...
                if (request.isForMainFrame()) view.loadUrl("file:///android_asset/offline.html?url="+Uri.encode(BuildConfig.MUSICROAD_URL));
            }
            @Override public void onPageFinished(WebView view,String url) {
                super.onPageFinished(view,url);
                view.evaluateJavascript("document.documentElement.classList.add('native-app');window.MusicRoadNativeInfo={platform:'android',version:'"+BuildConfig.VERSION_NAME+"',mediaStore:true,nativePlayer:true};document.dispatchEvent(new CustomEvent('mr:native-ready'));",null);
        ...
        webView.setWebChromeClient(new WebChromeClient() {
            ...
            @Override public boolean onShowFileChooser(WebView webView, ValueCallback<Uri[]> cb, FileChooserParams params) {
                ...
                try { startActivityForResult(Intent.createChooser(i,"Escolher músicas"),REQ_FILES); } catch (ActivityNotFoundException ex) { fileCallback.onReceiveValue(null); fileCallback=null; }
                return true;
            }
        });
    ...
    @Override protected void onActivityResult(int requestCode,int resultCode,Intent data){ super.onActivityResult(requestCode,resultCode,data); if(requestCode!=REQ_FILES||fileCallback==null)return; Uri[] results=null; if(resultCode==RESULT_OK&&data!=null){ if(data.getClipData()!=null){int count=data.getClipData().getItemCount();results=new Uri[count];for(int i=0;i<count;i++)results[i]=data.getClipData().getItemAt(i).getUri();} else if(data.getData()!=null)results=new Uri[]{data.getData()}; } fileCallback.onReceiveValue(results); fileCallback=null; }
    @Override public void onRequestPermissionsResult(int requestCode,String[] permissions,int[] grantResults){ super.onRequestPermissionsResult(requestCode,permissions,grantResults); if(requestCode==REQ_LOCATION&&geoCallback!=null){boolean g=grantResults.length>0&&grantResults[0]==PackageManager.PERMISSION_GRANTED;geoCallback.invoke(geoOrigin,g,false);geoCallback=null;geoOrigin=null;} else if(requestCode==REQ_AUDIO){boolean g=grantResults.length>0&&grantResults[0]==PackageManager.PERMISSION_GRANTED;dispatchAudioPermission(g);} }
    @Override protected void onDestroy(){ try{unregisterReceiver(playbackReceiver);}catch(Exception ignored){} if(webView!=null){webView.loadUrl("about:blank");webView.stopLoading();webView.removeJavascriptInterface("MusicRoadAndroid");webView.destroy();} super.onDestroy(); }
}
