...
public final class NavigationService extends Service implements LocationListener {
    ...
    private final Set<String> warnedApproach=new HashSet<>();
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        if(ACTION_START.equals(action)){
            ...
            warnedApproach.clear();warnedNow.clear();lastProgressM=0;offRouteSamples=0;suspendedOffRoute=false;persistState();
        ...
        if(route.isEmpty() && restoreState()){
            startForeground(NOTIF_TRIP,tripNotification("Rota offline restaurada"));startLocation();return START_STICKY;
        }
    ...
    private void createChannels(){
        ...
        NotificationChannel alert=new NotificationChannel(CH_ALERT,"Alertas de fiscalização",NotificationManager.IMPORTANCE_HIGH);alert.setDescription("Avisos temporários de radar, fiscalização, semáforo e quebra-mola");alert.setSound(null,null);alert.enableVibration(true);alert.setVibrationPattern(new long[]{0,120,80,120});
    ...
    private Notification tripNotification(String text){
        Notification.Builder b=builder(CH_TRIP).setSmallIcon(R.drawable.ic_notification).setContentTitle("MusicRoad · viagem ativa").setContentText(text).setContentIntent(openAppIntent()).setOngoing(true).setCategory(Notification.CATEGORY_NAVIGATION).setOnlyAlertOnce(true);
        if(Build.VERSION.SDK_INT<26)b.setPriority(Notification.PRIORITY_LOW);return b.build();
    }
    ...
    private void showTransient(String title,String text,String kind,long durationMs){
        Notification.Builder b=builder(CH_ALERT).setSmallIcon(R.drawable.ic_notification).setContentTitle(title).setContentText(text).setStyle(new Notification.BigTextStyle().bigText(text)).setContentIntent(openAppIntent()).setAutoCancel(true).setCategory(Notification.CATEGORY_NAVIGATION).setOnlyAlertOnce(false);
        if(Build.VERSION.SDK_INT>=26)b.setTimeoutAfter(durationMs);else b.setPriority(Notification.PRIORITY_HIGH);
        notificationManager.notify(NOTIF_ALERT,b.build());handler.removeCallbacksAndMessages("road-alert");handler.postAtTime(()->notificationManager.cancel(NOTIF_ALERT),"road-alert",System.currentTimeMillis()+durationMs);
    }

    private void startLocation(){
        if(Build.VERSION.SDK_INT>=23 && checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED && checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION)!=PackageManager.PERMISSION_GRANTED){showTransient("GPS necessário","Abra o MusicRoad e permita acesso à localização.","danger",6000);stopNavigation();return;}
    ...
    @Override public void onLocationChanged(Location loc){
        ...
        if(offRouteSamples>=3){
            if(!suspendedOffRoute){suspendedOffRoute=true;showTransient("Rota alterada","Você saiu do trajeto. O MusicRoad vai recalcular quando a tela estiver ativa.","danger",6500);speak("Atenção. Você saiu da rota. Recalculando trajeto.");}
        ...
        if(next!=null)handleHazard(next,progress,loc);
    ...
    private Hazard nextHazard(double progress,Location loc){
        ...
        for(Hazard h:hazards){
            double along=h.routeM-progress;if(along<-55||along>5000)continue;
            if(h.routeDistanceM>360)continue;
    ...
    private void handleHazard(Hazard h,double progress,Location loc){
        double distance=Math.max(0,h.routeM-progress);int radius=Math.max(h.alertRadiusM,h.kind().equals("portable")?700:350);
        String key=h.key;
        if(distance<=radius && distance>45 && !warnedApproach.contains(key)){
            warnedApproach.add(key);int rounded=Math.max(300,(int)(Math.round(distance/50.0)*50));String title=h.title();String text=h.text(rounded);String voice=h.voice(rounded);
            showTransient(title,text,h.kind(),6200);speak(voice);
        }
        if(h.isSpeedEnforcement() && distance<=40 && !warnedNow.contains(key)){
            warnedNow.add(key);String text=h.speed>0?"Radar agora · limite "+h.speed+" km/h":"Radar agora";showTransient("RADAR AGORA",text,"danger",4800);speak(h.speed>0?"Radar agora. Limite de "+h.speed+" quilômetros por hora.":"Radar agora.");
        }
    }

    private void handleSpeeding(long kmh,int limit){long now=System.currentTimeMillis();if(kmh<=limit+3||now-lastSpeedWarningAt<22000)return;lastSpeedWarningAt=now;showTransient("REDUZA A VELOCIDADE","Você está a "+kmh+" km/h · limite "+limit+" km/h","danger",5200);speak("Atenção. Reduza a velocidade. Limite de "+limit+" quilômetros por hora.");}
    ...
    private Projection project(double lat,double lon){Projection best=new Projection();best.offRouteM=Double.MAX_VALUE;best.routeM=0;best.segment=0;if(route.size()<2)return best;double latScale=110540.0;double lonScale=111320.0*Math.max(.2,Math.cos(Math.toRadians(lat)));for(int i=1;i<route.size();i++){RoutePoint a=route.get(i-1),b=route.get(i);double ax=(a.lon-lon)*lonScale,ay=(a.lat-lat)*latScale,bx=(b.lon-lon)*lonScale,by=(b.lat-lat)*latScale,vx=bx-ax,vy=by-ay,den=vx*vx+vy*vy,t=den>0?-(ax*vx+ay*vy)/den:0;t=Math.max(0,Math.min(1,t));double x=ax+t*vx,y=ay+t*vy,d=Math.sqrt(x*x+y*y);if(d<best.offRouteM){best.offRouteM=d;best.routeM=a.cumM+(b.cumM-a.cumM)*t;best.segment=i-1;}}return best;}
    ...
    private static String formatDistance(double m){return m>=1000?String.format(java.util.Locale.US,"%.1f km",m/1000.0):Math.max(0,(int)Math.round(m))+" m";}
    ...
    private static final class Projection{double offRouteM,routeM;int segment;}
    ...
    private static final class Hazard{
        String key,type,source;double lat,lon,routeM,routeDistanceM;int speed,alertRadiusM;float heading,routeBearing;
        String kind(){String t=type==null?"":type.toUpperCase(java.util.Locale.ROOT);if(t.contains("VIDEO")||t.contains("OCR")||t.contains("MONITOR"))return "video";if(t.contains("QUEBRA")||t.contains("LOMBADA")||t.contains("BUMP")||t.contains("HUMP")||t.contains("CALMING"))return "bump";if(t.contains("SEMAFOR")||t.contains("SEMÁFOR")||t.contains("SIGNAL")||t.contains("AVAN"))return "signal";if(t.contains("PORTAT")||t.contains("MOVEL")||t.contains("MÓVEL"))return "portable";return "radar";}
        boolean isSpeedEnforcement(){String k=kind();return k.equals("radar")||k.equals("portable");}
        String label(){String k=kind();if(k.equals("video"))return "Videomonitoramento";if(k.equals("bump"))return "Quebra-mola";if(k.equals("signal"))return type!=null&&type.toUpperCase(java.util.Locale.ROOT).contains("FISCALIZ")?"Fiscalização semafórica":"Semáforo";if(k.equals("portable"))return "Fiscalização portátil";return "Radar";}
        String title(){String k=kind();if(k.equals("video"))return "ÁREA MONITORADA";if(k.equals("bump"))return "QUEBRA-MOLA À FRENTE";if(k.equals("signal"))return label().toUpperCase(java.util.Locale.ROOT);if(k.equals("portable"))return "FISCALIZAÇÃO PORTÁTIL";return "RADAR À FRENTE";}
        String text(int meters){return label()+" a aproximadamente "+meters+" m"+(isSpeedEnforcement()&&speed>0?" · limite "+speed+" km/h":"");}
        String voice(int meters){String base="Atenção. "+label()+" a aproximadamente "+meters+" metros.";if(isSpeedEnforcement()&&speed>0)base+=" Limite de "+speed+" quilômetros por hora.";return base;}
    }
}
